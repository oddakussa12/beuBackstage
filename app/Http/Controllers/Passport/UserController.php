...
//                Log::info('test' , array('$start:'.$start.' $num:'.$num.' $tomorrowNum:'.$tomorrowNum.' $twoNum:'.$twoNum.' $threeNum:'.$threeNum.' $sevenNum:'.$sevenNum.' $thirtyNum:'.$thirtyNum));
    ...
    public function yesterday(Request $request)
    {
        ...
        $dau = $connection->table('dau_counts')->where('date' , $date)->groupBy('date')->select(DB::raw("sum(`dau`) as `dau`"))->first()->map(function ($value) {return (array)$value;})->toArray();
        $list['dau'] = array(
            'date'=>$date,
            'dau'=>$dau,
        );
        ...
        $oneKeep = $connection->table('data_retentions')->where('date' , $one)->select('date' , DB::raw("sum(`1`) as `one`"))->first()->map(function ($value) {return (array)$value;})->toArray();
        $list['keep']['one'] = array(
            'one'=>$one,
            'oneKeep'=>$oneKeep,
        );
        ...
        $twoKeep = $connection->table('data_retentions')->where('date' , $two)->select('date' , DB::raw("sum(`2`) as `two`"))->first()->map(function ($value) {return (array)$value;})->toArray();
        $list['keep']['two'] = array(
            'two'=>$two,
            'twoKeep'=>$twoKeep,
        );
        ...
        $threeKeep = $connection->table('data_retentions')->where('date' , $three)->select('date' , DB::raw("sum(`3`) as `three`"))->first()->map(function ($value) {return (array)$value;})->toArray();
        ...
        $sevenKeep = $connection->table('data_retentions')->where('date' , $seven)->select('date' , DB::raw("sum(`7`) as `seven`"))->first()->map(function ($value) {return (array)$value;})->toArray();
        ...
        $fourteenKeep = $connection->table('data_retentions')->where('date' , $fourteen)->select('date' , DB::raw("sum(`14`) as `fourteen`"))->first()->map(function ($value) {return (array)$value;})->toArray();
        ...
        $thirtyKeep = $connection->table('data_retentions')->where('date' , $thirty)->select('date' , DB::raw("sum(`30`) as `thirty`"))->first()->map(function ($value) {return (array)$value;})->toArray();
