    ...
    <div  class="layui-fluid">
        <table class="layui-table" lay-filter="post_table">
                ...
                <tr><th>
                        <div>
                            <form id="form-refuse" method="post">
                                {{ csrf_field() }}
                                <input name="status" hidden value="block">
                                <input name="id" hidden value="{{$result['post_id']}}">
                                <input name="userId" hidden value="{{$result['user_id']}}">
                                <button class="layui-btn layui-btn-warm" type="submit" lay-submit="">不通过并封号</button>
                            </form>
                        </div>
                        <div>
                            <form id="form-refuse" method="post">
                        ...
                        <div>
                            <form id="form-pass" method="post">
                                {{ csrf_field() }}
                                <input name="status" hidden value="recover">
                                <input name="uuid" hidden value="{{$result['post_uuid']}}">
                                <input name="id" hidden value="{{$result['post_id']}}">
                                <button class="layui-btn layui-btn-danger" type="submit" lay-tips="恢复" lay-submit="">恢复</button>
                            </form>
                        </div>
                        <div style="position: absolute;margin-left: 80%; line-height: 40px;">
                            <span class="layui-btn layui-btn-normal" style="display: block; cursor:default">
                               <i> @if ($result['post_type']=='vote')  投票帖 @endif
                                @if ($result['post_type']=='image') 图片帖 @endif
                                @if ($result['post_type']=='video') 视频帖 @endif
                                @if ($result['post_type']=='text') 文本帖 @endif</i>
                                待审核数量：<i style="font-weight: 600;font-size: 16px;">{{$result['totalCount']}}</i>
                                </span>
                        </div>
                    </th>
                </tr>
            ...
            <tr style="display: none"><td>发布时间： {{date('Y-m-d H:i:s',strtotime($result['post_created_at'])+3600*8)}}</td></tr>
                <tr><td><span style="display: block; margin-right: 50px;"><b>USER_ID：</b>{{$result['user_id']}}
                        ...
                        <span style="margin-right: 20px;">POST_ID：{{$result['post_id']}}
                            <a class="layui-btn layui-btn-danger layui-btn-xs" lay-event="people" target="_blank" href="{{route('report::report.reportUser')}}?reportable_id={{$result['post_id']}}">举报人</a>
                        </span>
                        <span> POST_UUID：
                        ...
                        <span style="margin-left: 10%">
                            <span class="layui-btn layui-btn-normal" style="height: 22px; line-height: 22px;">@if(!empty($result['post_deleted_at']))已删除@else 未删除@endif</span>
                ...
                @endif
                        ...
                        <span class="layui-btn layui-btn-checked" style=" cursor:default; height: 20px;line-height: 20px;float: right; margin-right:20px;">
                             <i style="font-size: 12px; padding-right: 50px;">
                                 被举报次数：<i style="font-size: 14px; font-weight: 600; color: red">{{$result['reportTimes']}} </i>
