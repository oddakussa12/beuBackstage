...
include_once ('com/umeng/uapp/param/UmengUappGetLaunchesParam.class.php');
include_once ('com/umeng/uapp/param/UmengUappGetLaunchesResult.class.php');
...
try {
    ...
    if($type==2)
    {
        $android = getResult(1)->getLaunchInfo();
        $ios = getResult(0)->getLaunchInfo();
        echo json_encode(array('result'=>array('ios'=>$ios , 'android'=>$android)));
    }else{
        echo json_encode(getResult($type)->getLaunchInfo());
    }
    die;
    // ----------------------------example end-------------------------------------
