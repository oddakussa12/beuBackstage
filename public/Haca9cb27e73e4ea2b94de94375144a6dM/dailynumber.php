...
include_once ('com/umeng/uapp/param/UmengUappGetActiveUsersParam.class.php');
include_once ('com/umeng/uapp/param/UmengUappGetActiveUsersResult.class.php');
...
try {
    ...
    if($type==2)
    {
        $android = getResult(1)->getActiveUserInfo();
        $ios = getResult(0)->getActiveUserInfo();
        echo json_encode(array('result'=>array('ios'=>$ios , 'android'=>$android)));
    }else{
        echo json_encode(getResult($type)->getActiveUserInfo());
    }
    die;
    // ----------------------------example end-------------------------------------
